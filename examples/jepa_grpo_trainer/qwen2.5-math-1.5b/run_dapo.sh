#!/usr/bin/env bash
# DAPO baseline (JEPA OFF) | Qwen2.5-Math-1.5B-Instruct | Ray + FSDP + hybrid-engine vLLM
# Pure DAPO: JEPA auxiliary loss disabled and no Code view -- all 8
# rollouts/prompt are CoT-framed and feed the DAPO policy loss.
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-dapo-${MODEL_NAME}-${RUN_TIMESTAMP}}
