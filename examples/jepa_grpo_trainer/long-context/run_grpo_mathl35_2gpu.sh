...
# PLAIN GRPO, 2 GPUs, Hendrycks MATH Level 3-5 -- the dataset the GXPO/SFPO arms used.
#   model : Qwen2.5-Math-1.5B-Instruct
#   data  : /workspace/jepa-grpo-cache/data/math_l35/train.parquet  (5586 rows, MATH L3-5)
#   jepa.enable=False -> no auxiliary loss. Clean GRPO reference.
#
# Both GPUs train (data parallel, n_gpus_per_node=2, fsdp_size=1 => NO_SHARD replication,
# right for 1.5B on 94GB cards). Validation is back INLINE (test_freq=10); the async
# eval-on-a-second-GPU experiment is dropped.
#
# validation_seeds is ONE seed, not three. The val set now includes math500 (500 problems),
# so the 4-benchmark set is 600 problems; at 3 seeds that is 14.4k generations per validation
# and would dominate wall-clock again. 500 problems at 1 seed is far more precise than the
# old 40-problem amc23 at 3 seeds ever was, so the seeds are the right thing to cut.
exec /workspace/exploration/.venv/bin/python3 -m verl.experimental.jepa_grpo.main_ray \
  ...
  trainer.experiment_name=grpo-mathl35-Qwen2.5-Math-1.5B-2gpu-20260726 \
  trainer.default_local_dir=checkpoints/grpo-qwen-3b/grpo-mathl35-Qwen2.5-Math-1.5B-2gpu-20260726 \
