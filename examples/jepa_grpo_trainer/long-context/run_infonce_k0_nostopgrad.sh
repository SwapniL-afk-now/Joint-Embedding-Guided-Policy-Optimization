...
# predictor_k=0 (IDENTITY predictor Pred(x)=x: no appended [PRED] token, every
# read is the last CONTENT token; p+/p- differ only by rollout correctness) +
# joint correct/teacher attraction with no stop-grad. Wrong-response repulsion
# treats correct/teacher as detached references so only wrong moves away.
...
exec /workspace/exploration/.venv/bin/python3 -m verl.experimental.jepa_grpo.main_ray \
  ...
  trainer.experiment_name=geometry-k0-patience-Qwen2.5-3B-Instruct-20260722 \
  trainer.default_local_dir=checkpoints/grpo-qwen-3b/geometry-k0-patience-Qwen2.5-3B-Instruct-20260722 \
