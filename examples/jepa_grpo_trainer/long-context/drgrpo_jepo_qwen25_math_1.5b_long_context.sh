...
# DeepScaleR-Preview subset (40,309 rows). Teacher CoT+Code caches were generated
# with examples/jepa_grpo_trainer/gen_teacher_caches.py against this exact parquet
# (Qwen2.5-14B-Instruct, code view retried with Qwen2.5-Coder-14B-Instruct) and
# merged to the ds40k.{cot,code}.responses.pt caches referenced below.
DS40K_DIR=${DS40K_DIR:-/workspace/jepa-grpo-cache}
TRAIN_FILE=${TRAIN_FILE:-${DS40K_DIR}/data/deepscaler_preview_train.parquet}
NNODES=${NNODES:-1}
...
# ALPHA = the paper's λ. RL policy and JEPA are applied as TWO sequential optimizer
# steps (actor step, then jepa step; see ray_trainer Step 4/5 + worker.jepa_update),
# NOT a fused summed step. Measured on this run (2xB200, deepscaler): actor grad
# ‖g_policy‖≈0.016, raw ‖∂L_JEPA‖≈0.12. Because both steps share ONE Adam optimizer,
# a JEPA grad much smaller than the policy grad is drowned by leftover policy
# momentum (its step ≈ re-applied policy momentum), so α must be large enough that
...
MIN_VALID_PAIRS=${MIN_VALID_PAIRS:-1}
# Default loss mode: llm-jepa-contrastive (paper-exact llm-jepa arm A, PLUS two
# arms that also use the student's WRONG rollouts: (B) align a prompt's wrong
# rollouts to one chosen wrong response via a <|bad_predictor_i|> read, and
# (C) push the <good_pred>/<bad_pred> reads apart for prompts with both).
# Requires LLM_JEPA_PREDICTOR_K>0 (needs the pred reads) — set below.
#   JEPA_LOSS_TYPE=llm-jepa  reverts to the paper-exact single-arm loss.
JEPA_LOSS_TYPE=${JEPA_LOSS_TYPE:-llm-jepa-contrastive}
...
# SIGReg anti-collapse for llm-jepa-contrastive (convex: L=(1-λ)·arms+λ·SIGReg).
# RL has no supervised LM loss to ground the reads (unlike the official SFT LLM-JEPA),
# so the cosine-alignment arms collapse the read space into one cone; SIGReg (LeJEPA's
# fix for the no-supervised-anchor regime) spreads it back out. λ=0 disables it.
SIGREG_LAMBDA=${SIGREG_LAMBDA:-0.1}         # convex mix weight (0..1)
# SIGReg Epps-Pulley knobs (LeJEPA defaults arXiv 2511.08544 §6.1); rarely need tuning.
SIGREG_M=${SIGREG_M:-1024}                  # random projection directions
SIGREG_T_MIN=${SIGREG_T_MIN:--5.0}          # CF frequency range min
SIGREG_T_MAX=${SIGREG_T_MAX:-5.0}           # CF frequency range max
SIGREG_S=${SIGREG_S:-1.0}                    # Gaussian kernel bandwidth
# Teacher CoT view = pregenerated verified-correct CoT solutions from Qwen2.5-14B-
# Instruct, generated by gen_teacher_caches.py against deepscaler_preview_train.parquet
# (2 shards, data-parallel) and merged with --merge. 24,014/40,309 prompts covered.
TEACHER_CACHE=${TEACHER_CACHE:-${DS40K_DIR}/ds40k.cot.responses.pt}
# Teacher Code view = SAME dataset, code view, generator retried on still-short
# prompts with Qwen2.5-Coder-14B-Instruct (gen_teacher_caches.py --views code
# --retry-rounds 0), then merged. REQUIRED by llm-jepa / llm-jepa-contrastive.
# 18,083/40,309 prompts covered.
CODE_TEACHER_CACHE=${CODE_TEACHER_CACHE:-${DS40K_DIR}/ds40k.code.responses.pt}
...
# Off by default: keep JEPA active for the ENTIRE run. The plateau latch was
# euthanizing JEPA mid-run (~step 87) once alignment cosines flattened; we want the
# contrastive + SIGReg signal to keep training the whole time.
AUTO_OFF_ENABLE=${AUTO_OFF_ENABLE:-False}
...
JEPA=(
    ...
    jepa.triplet_sigreg_lambda=${SIGREG_LAMBDA}
    jepa.n_projections=${SIGREG_M}
    jepa.t_min=${SIGREG_T_MIN}
    jepa.t_max=${SIGREG_T_MAX}
    jepa.epps_pulley_s=${SIGREG_S}
