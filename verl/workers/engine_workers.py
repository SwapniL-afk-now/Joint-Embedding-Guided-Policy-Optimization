...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def train_mini_batch(self, data: TensorDict) -> TensorDict:
        ...
        # apply_step=False defers every mini-batch's optimizer step in this call to a caller
    ...
    def train_batch(self, data: TensorDict) -> TensorDict:
        ...
        # apply_step=False lets a caller accumulate this batch's gradient without stepping the
        ...
        update_lr_scheduler = apply_step and tu.get(data, key="update_lr_scheduler", default=False)
        # update lr scheduler
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(
        self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None, tag="default", save_contents=None
    ):
        return self.engine.save_checkpoint(
            local_path, hdfs_path, global_step, max_ckpt_to_keep, tag=tag, save_contents=save_contents
        )
