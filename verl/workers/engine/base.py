...
class BaseEngine:
    ...
    def train_batch(self, data: TensorDict, loss_function: Callable, apply_step: bool = True) -> Any:
        ...
        Args:
            ...
            apply_step: If False, zero the gradient and accumulate this batch's backward pass onto
                it as usual, but skip `optimizer_step()` — the gradient is left in place for a
                caller to add further backward passes onto (e.g. an auxiliary loss on the same
