#!/usr/bin/env bash
# Dr.GRPO + JEPA (jepa-tcr-dual) | Qwen2.5-Math-1.5B-Instruct | Ray + FSDP + hybrid-engine vLLM
...
# NOTE: Qwen2.5-Math has a 4096-token context window; the defaults below
# (max_prompt 1024 + max_response 3072) exactly fill it -- do not raise
# MAX_RESPONSE_LENGTH without also lowering MAX_PROMPT_LENGTH.
...
#
# Before first run:
#   make sure the model is at /workspace/models/Qwen2.5-Math-1.5B-Instruct
...
########################### user-adjustable ###########################
EXPECTED_MODEL_BASENAME=${EXPECTED_MODEL_BASENAME:-Qwen2.5-Math-1.5B-Instruct}
...
MAX_OPTIMIZER_STEPS=${MAX_OPTIMIZER_STEPS:-800}   # legacy fallback for older launchers
...
# One trainer iteration consumes one rollout batch; JEPA may take a second
# optimizer step within that same batch when fuse_optimizer_step=False.
TOTAL_TRAINING_BATCHES=${TOTAL_TRAINING_BATCHES:-${TOTAL_TRAINING_STEPS:-${MAX_OPTIMIZER_STEPS}}}
...
if [[ "$(basename "${MODEL_PATH}")" != "${EXPECTED_MODEL_BASENAME}" && "${ALLOW_MODEL_PATH_OVERRIDE}" != "true" ]]; then
    echo "Refusing to run qwen2.5-math-1.5b recipe with MODEL_PATH=${MODEL_PATH}" >&2
...
TRAINER=(
    ...
    trainer.total_training_steps=${TOTAL_TRAINING_BATCHES}
